...
const preloadModules = new Map([
  ...
  ['slimcore/lib/video-renderer', require('video_renderer')],
  ['electron', electron],
  ['fs', fs],
  ['events', events],
  ['os', require('os')],
...
if (binding.getMallocMemoryUsage) {
  preloadProcess.getMallocMemoryUsage = (maxSize, maxCount) => binding.getMallocMemoryUsage(maxSize, maxCount)
}
preloadProcess.startWatchdog = (timeout) => binding.startWatchdog(timeout)
preloadProcess.stopWatchdog = () => binding.stopWatchdog()
...
global.WeakReference = binding.WeakReference
